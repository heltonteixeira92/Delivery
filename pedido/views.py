from django.db.models.fields import CommaSeparatedIntegerField # noqa
...
def finalizar_pedido(request):
    if request.method == "GET":
        ...
        if len(request.session['carrinho']) > 0:
            ...
            lambda_func_troco = lambda x: int(x['troco_para']) - total if not x['troco_para'] == '' else "" # noqa
            lambda_func_pagamento = lambda x: 'Cartão' if x['meio_pagamento'] == '2' else 'Dinheiro' # noqa
